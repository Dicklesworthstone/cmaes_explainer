...
- Physics is **Rust→wasm-pack, committed per-version, fail-closed version-gated** (`AUDITED_CMAES_KERNEL_VERSION` 0.4.1; current G1/arm owner package 0.6.11; refusal envelopes, never silent TS fallback) — this is the mega-kernel doctrine's parity-first culture already in miniature.
