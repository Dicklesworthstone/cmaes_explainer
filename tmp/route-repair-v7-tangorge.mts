...
const R_BODY = R - 0.002;
function bodyInWall(x: number, y: number): boolean {
  for (const w of CRAFTSMAN_BUNGALOW_1928.walls) {
    ...
    if (Math.hypot(x - px, y - py) > w.thickness / 2 + R_BODY) continue;
    let inDoor = false;
...
for (let j = 0; j < NY; j++)
  for (let i = 0; i < NX; i++) {
    ...
    free[j * NX + i] = !bodyInWall(x, y) && clearanceAt(x, y) >= FREE_MIN ? 1 : 0;
  }
...
function relocate(ox: number, oy: number): { pos: [number, number]; moved: number; clr: number } {
  ...
  if (clearanceAt(ox, oy) >= 0.02 && !bodyInWall(ox, oy) && cand(ox, oy))
  ...
  for (let rad = 0.1; rad <= 1.0; rad += 0.1)
    for (let a = 0; a < 360; a += 8) {
      ...
      if (!cand(x, y) || bodyInWall(x, y)) continue;
...
for (const route of CRAFTSMAN_WALKING_ROUTES) {
  ...
  const gates: Array<{ name: string; pos: [number, number]; speed: number }> = [];
  let failed = false;
  for (let i = 0; i < stands.length - 1; i++) {
    ...
    const simpleSeg = simplify(seg);
    if (i === 0) {
      gates.push({ name: route.waypoints[0].name, pos: [+stands[0].pos[0].toFixed(3), +stands[0].pos[1].toFixed(3)], speed: route.waypoints[0].speed });
    }
    for (const p of simpleSeg.slice(1, -1))
      gates.push({ name: "via", pos: [+p[0].toFixed(3), +p[1].toFixed(3)], speed: 0.5 });
    gates.push({ name: route.waypoints[i + 1].name, pos: [+stands[i + 1].pos[0].toFixed(3), +stands[i + 1].pos[1].toFixed(3)], speed: route.waypoints[i + 1].speed });
  }
  if (failed) continue;
  let viaN = 0;
