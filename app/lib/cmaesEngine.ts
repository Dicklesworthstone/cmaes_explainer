...
function assertSameLength(a: Vector, b: Vector, operation: string): void {
  if (a.length !== b.length) {
    throw new RangeError(
      `${operation} requires vectors of equal length; received ${a.length} and ${b.length}.`,
    );
  }
}
...
export function matVecMult(matrix: Matrix, vector: Vector): Vector {
  if (matrix.length === 0 || matrix.some((row) => row.length !== vector.length)) {
    throw new RangeError(
      "matVecMult requires a non-empty matrix whose row width matches the vector length.",
    );
  }
...
function nextOpenUnit(rng: () => number): number {
  ...
  throw new RangeError(
    "Gaussian sampling requires an RNG that produces values strictly between 0 and 1.",
  );
}
...
export function sampleGaussianVector(dim: number, rng: () => number): Vector {
  if (!Number.isInteger(dim) || dim < 1)
    throw new RangeError("sampleGaussianVector requires a positive integer dimension.");
...
export const BENCHMARKS: BenchmarkFunction[] = [
  {
    ...
    formula:
      "f(x, y) = 100(\\textcolor{#60a5fa}{y} - \\textcolor{#60a5fa}{x}^2)^2 + (1 - \\textcolor{#60a5fa}{x})^2",
    domain: [-2.5, 2.5],
    optimum: [1, 1],
    optimumValue: 0,
  ...
  },
  {
    ...
    category: "multimodal",
    description:
      "A regular array of local minima surrounding the global basin. It exposes the local-search nature of one CMA-ES run and motivates independent restarts with larger populations.",
    formula:
  ...
  },
  {
    ...
    category: "multimodal",
    description:
      "A broad outer plateau, periodic ripples, and a narrow central basin make both global scale selection and local refinement important.",
    formula:
      "f(x, y) = -20\\exp(-0.2\\sqrt{0.5(\\textcolor{#60a5fa}{x}^2+\\textcolor{#60a5fa}{y}^2)}) - \\exp(0.5[\\cos(2\\pi \\textcolor{#60a5fa}{x})+\\cos(2\\pi \\textcolor{#60a5fa}{y})]) + 20 + e",
    domain: [-3, 3],
    optimum: [0, 0],
    optimumValue: 0,
    eval: (x, y) => {
  ...
  },
  {
    ...
    category: "ill-conditioned",
    description:
      "The Hessian is 1000 times steeper in y than in x, so the level-set axis ratio is √1000 ≈ 31.6. CMA-ES must learn this anisotropy instead of using one scale in both directions.",
  ...
  },
  {
    ...
    category: "multimodal",
    description:
      "Four separated global minima have equal objective value, so the basin reached by one local run depends on its initial distribution and samples.",
    formula:
      "f(x, y) = (\\textcolor{#60a5fa}{x}^2 + \\textcolor{#60a5fa}{y} - 11)^2 + (\\textcolor{#60a5fa}{x} + \\textcolor{#60a5fa}{y}^2 - 7)^2",
    domain: [-4, 4],
    optimum: [3, 2],
    optima: [
      [3, 2],
  ...
  },
  {
    ...
    description:
      "A piecewise-flat radial staircase plus a nonsmooth diagonal ridge makes local finite-difference information brittle while rank comparisons remain usable. Its zero-valued global set is the segment x = y inside the unit disk; the origin marker is one representative optimum.",
    formula:
      "f(x, y) = \\lfloor \\textcolor{#60a5fa}{x}^2 + \\textcolor{#60a5fa}{y}^2 \\rfloor + 0.5|\\textcolor{#60a5fa}{x} - \\textcolor{#60a5fa}{y}|",
    domain: [-2.5, 2.5],
    optimum: [0, 0],
    optimumValue: 0,
...
function safeObjectiveValue(value: number): number {
  if (Number.isNaN(value) || value === -Infinity) {
    throw new RangeError(
      "The objective returned NaN or -Infinity; candidates must have an orderable minimization score.",
    );
  }
  return value;
}
...
export class CMAESOptimizer {
  ...
  constructor(
    private readonly objective: (x: number, y: number) => number,
    options: CMAESOptions = {},
  ) {
    const requestedDim = options.dim ?? 2;
    if (requestedDim !== 2) {
      throw new RangeError(
        `CMAESOptimizer is the explainer's 2-D engine; received dim=${requestedDim}. Use CMAESOptimizerND for N-D problems.`,
      );
    }
    ...
    if (!Number.isFinite(this.sigma) || this.sigma <= 0)
      throw new RangeError("initialSigma must be a finite positive number.");

    this.lambda = options.lambda ?? 4 + Math.floor(3 * Math.log(this.dim));
    if (!Number.isInteger(this.lambda) || this.lambda < 2)
      throw new RangeError("lambda must be an integer of at least 2.");

    this.activeCMA = options.activeCMA ?? true;
    this.noiseLevel = options.noiseLevel ?? 0;
    if (!Number.isFinite(this.noiseLevel) || this.noiseLevel < 0)
      throw new RangeError("noiseLevel must be a finite non-negative number.");
    ...
    this.cmu = Math.min(
      1 - this.c1,
      (2 * (this.mueff - 2 + 1 / this.mueff)) / ((this.dim + 2) ** 2 + this.mueff),
    );
  ...
  step(): CMAESGenerationState {
    ...
    const hSigma =
      pSigmaNorm / Math.max(Number.EPSILON, pathNormalizer) / this.chiN < 1.4 + 2 / (this.dim + 1)
        ? 1
        : 0;
    ...
    const oldCoefficient =
      1 + this.c1 * deltaHSigma - this.c1 - this.cmu * this.covarianceWeightSum;
    const provisionalC00 =
      oldCoefficient * this.C[0][0] + this.c1 * this.pC[0] ** 2 + this.cmu * rankMu00;
    const provisionalC01 =
      oldCoefficient * this.C[0][1] + this.c1 * this.pC[0] * this.pC[1] + this.cmu * rankMu01;
    const provisionalC11 =
      oldCoefficient * this.C[1][1] + this.c1 * this.pC[1] ** 2 + this.cmu * rankMu11;
...
export function runGradientDescent(
  ...
  if (!Number.isFinite(lr) || lr <= 0 || !Number.isFinite(eps) || eps <= 0)
    throw new RangeError("lr and eps must be finite positive numbers.");
  if (!Number.isFinite(maxStepNorm) || maxStepNorm <= 0)
    throw new RangeError("maxStepNorm must be a finite positive number.");
  ...
  const history: BaselineStepState[] = [
    { step: 0, currentX: [...x], bestX: [...bestX], bestFitness, evalCount },
  ];
...
export function runRandomSearch(
  ...
  if (!domain.every(Number.isFinite) || domain[0] >= domain[1])
    throw new RangeError("domain must contain finite min and max values with min < max.");
  if (!Number.isInteger(totalEvals) || totalEvals < 1)
    throw new RangeError("totalEvals must be a positive integer.");
