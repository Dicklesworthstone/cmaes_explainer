...
export const HONESTY_CHIP_REGISTRY: HonestyChip[] = [
  ...
  },
  {
    ...
    mathFormula: "Q(x,u) = \\frac12\\left[\\begin{smallmatrix}1\\\\1\\end{smallmatrix}\\right]^T \\begin{pmatrix}Q_x & Q_{xu} \\\\ Q_{xu}^T & Q_u\\end{pmatrix} \\begin{smallmatrix}1\\\\1\\end{smallmatrix} + \\frac12\\,\\mathrm{cost}_{xu} \\mathrm{cost}_x^T; \\quad k_t = -Q_{uu}^{-1} \\ell_u, \\quad K_t = -Q_{uu}^{-1} Q_{ux} \\mathrm{(backward\\ Riccati)}",
    claim: "Box-FDDP with Levenberg-Marquardt damping and Armijo backtracking line search produces a second-order-accurate local trajectory optimum; the inverse dynamics is replaced by featherstone ABA so a multi-link plan converges without any matrix-inversion outside the explicit Q_uu Cholesky.",
    status: "verified",
  },
  {
    id: "chip-cito",
    ...
    claim: "Direct collocation with smoothed Fischer-Burmeister complementarity discovers contact modes and produces whole-body obstacle avoidance without pre-defined contact schedules; the homotopy in \\varepsilon tightens the relaxation to the exact non-smooth problem.",
    status: "verified",
  },
];
