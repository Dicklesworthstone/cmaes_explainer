...
export function ConvergenceChart({
  ...
  const chart = useMemo(() => {
    ...
    const yso = (v: number) =>
      m.top + ch - ((v - (objMin - objPad)) / (objRange + 2 * objPad)) * ch;
    const yss = (v: number) => m.top + ch - ((v - sigMin) / (sigRange + 1e-10)) * ch;
    const objPath = data
      .map(
        (d, i) =>
          `${i === 0 ? "M" : "L"}${xs(d.generation).toFixed(1)},${yso(d.bestObjective).toFixed(1)}`,
      )
      .join(" ");
    const sigPath = data
      .map(
        (d, i) => `${i === 0 ? "M" : "L"}${xs(d.generation).toFixed(1)},${yss(d.sigma).toFixed(1)}`,
      )
      .join(" ");
    const areaPath =
      objPath +
      ` L${xs(genMax).toFixed(1)},${(m.top + ch).toFixed(1)} L${xs(genMin).toFixed(1)},${(m.top + ch).toFixed(1)} Z`;
  ...
  if (!chart) {
    return (
      <div
        className="flex items-center justify-center rounded-xl border border-white/10 bg-slate-900/60 text-xs text-slate-500"
        style={{ width, height }}
      >
